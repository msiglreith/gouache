...
void main() {
    vec2 footprint = 1.1283791671 * vec2(length(vec2(dFdx(v_uv.x), dFdy(v_uv.x))), length(vec2(dFdx(v_uv.y), dFdy(v_uv.y))));
    ...
    for (uint i = v_path.x; i < v_path.y; i++) {
        ...
        float x_overlap = clamp((0.5 * footprint.x + (x - v_uv.x) / max(1.0, abs(dxdy))) / footprint.x, 0.0, 1.0);

        alpha += x_overlap * y_overlap;
    }

    f_col = vec4(clamp(alpha * alpha, 0.0, 1.0));
}
