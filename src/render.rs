...
impl Renderer {
    ...
    pub fn create_tex(&mut self, format: TextureFormat, width: usize, height: usize, pixels: &[u8]) -> TextureId {
    ...
    pub fn update_tex(&mut self, texture: TextureId, x: usize, y: usize, width: usize, height: usize, pixels: &[u8]) {
        self.textures.get_mut(&texture).unwrap().update(x, y, width, height, pixels);
    }

    pub fn delete_tex(&mut self, texture: TextureId) {
